...
chkAll.change(function(){
    ...
    // for문 없이도 여러개의 체크박스를 <?php include "inc/footer_area.inc" ?>
    // 동시에 변경할 수 있음! 
        ...
        if(!vReg(cv,cid)){ // 아이디검사 불통과시 들어감(!NOT)
            ...
            */
                ...
                $.ajax({
                    ...
                    // -> 비동기옵션은 본처리를 비동기적으로
                    // 처리하겠다는 것임(기본값:true)
                    // false 로 해야 동기화처리가 되어
                    // 아이디가 같은 게 있다면, 불통과 처리를 시킬 수 있는
                    // pass = false 를 처리할 수 있다
                    async:false,
                    // 6.성공처리(success)
...
.on('keyup',function(){
  ...
  *********************************************/
    ...
    $('#btnj').click(e=>{
        ...
        // 4. 검사결과에 따라 메시지 보이기
        if(pass){
            // 오리지널 포스트 방식으로 전송함 - 깜빡거림
            // $('.logF').submit();
            // 현재 페이지 form 정보가 모두 process/ins.php 로
            // 이동하여 데이터를 처리함  - 동기화방식

            // -> 현재 페이지를 가만히 두고 처리 페이지로, 
            // 비동기적인 처리를 하는 것이 바로 Ajax! -  안 깜빡거림
            ...
            /****************************************************
                ...
                - 비동기통신이란? 쉽게 말해서 페이지가 새로고쳐지지 
                않고 그대로 있으면서 일부분만 서버통신을 하는 것을 말한다
                - 제이쿼리는 POST방식으로 ajax를 처리하는 메서드를 제공한다

                [POST방식 Ajax 메서드]
                $.post(URL,data,callback)
                $.post(전송할 페이지,전송할 데이터,전송후 콜백함수)
